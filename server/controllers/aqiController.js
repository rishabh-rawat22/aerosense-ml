const { getLatestForCity, getActiveCities, LocationAQI, aqiToCategory } = require('../services/openaqService');
...
const extractLocation = (query) => {
  ...
  if (district) {
    ...
    if (/[^a-zA-Z\s\-'.]/.test(c)) throw { status: 400, message: 'District name contains invalid characters.' };
  ...
  if (lat !== undefined && lon !== undefined) {
    ...
    if (isNaN(la) || isNaN(lo)) throw { status: 400, message: 'Coordinates must be valid numbers.' };
...
// 30-day daily-average history for a city
const get30DayHistory = async (city) => {
  const rx       = new RegExp(city.trim(), 'i');
  const thirtyAgo = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);

  const rows = await LocationAQI.aggregate([
    { $match: { city: rx, recordedAt: { $gte: thirtyAgo }, aqi: { $gt: 0 } } },
    {
      $group: {
...
const getAdvisory = (categoryLabel) => {
  const map = {
    'Good':        { icon:'🟢', general:'Air quality is excellent. Perfect for all outdoor activities.',         sensitive:'No precautions needed.',                                                           outdoor:'Ideal for outdoor exercise.' },
    'Satisfactory':{ icon:'🟡', general:'Air quality is acceptable for most people.',                            sensitive:'Unusually sensitive individuals may experience mild discomfort.',                    outdoor:'Suitable for moderate outdoor activity.' },
    'Moderate':    { icon:'🟠', general:'Sensitive groups may experience health effects.',                       sensitive:'People with heart or lung disease, elderly and children should reduce exertion.',    outdoor:'Consider reducing intense outdoor activity.' },
    'Poor':        { icon:'🔴', general:'Everyone may begin to experience health effects.',                      sensitive:'Wear N95 masks outdoors. Sensitive groups should stay indoors.',                    outdoor:'Avoid prolonged outdoor exertion. Keep windows closed.' },
    'Very Poor':   { icon:'🟤', general:'Health warnings for the entire population.',                            sensitive:'Stay indoors and keep activity low. Serious health effects possible.',               outdoor:'Avoid all outdoor physical activity. Use air purifiers.' },
    'Severe':      { icon:'⚫', general:'Emergency conditions. Population at significant risk.',                 sensitive:'Do not go outside under any circumstances.',                                        outdoor:'Stay indoors, seal windows, use air purifiers at max. Seek medical help if symptomatic.' },
  };
...
const getCurrentAQI = async (req, res) => {
  try {
    ...
    const data = {
      ...
      dataAge:      Math.round((Date.now() - new Date(reading.recordedAt)) / 60000) + ' minutes ago',
...
const getForecast = async (req, res) => {
  try {
    ...
    const reading = await getLatestForCity(district);
    if (!reading) return res.status(404).json({ success: false, error: `No data for "${district}".` });

    const forecast = await get48HourForecast(
      district, reading.aqi, reading.pollutants,
      reading.coordinates?.lat, reading.coordinates?.lon
    );
...
const getHistoricalData = async (req, res) => {
  try {
    const { district } = extractLocation(req.query);
    ...
    if (!history.length) {
      return res.status(404).json({
        success: false,
        error: `No historical data for "${district}" yet. Data accumulates hourly.`,
      });
    }

    const data = { district, history, avgAccuracy: null, dataPoints: history.length, source: 'OpenAQ — openaq.org' };
...
const getDashboard = async (req, res) => {
  try {
    ...
    const [forecast, history] = await Promise.all([
      get48HourForecast(district, reading.aqi, reading.pollutants, reading.coordinates?.lat, reading.coordinates?.lon),
    ...
    const data = {
      ...
      forecast: forecast.map((f) => ({ ...f, category: getCategory(f.aqi) })),
      history:  { history, avgAccuracy: null, source: 'OpenAQ — openaq.org' },
      meta: {
        source:      'OpenAQ',
        generatedAt: new Date().toISOString(),
...
const getDistricts = async (req, res) => {
  try {
    ...
    // Normalise to { name, state, stations } so the frontend DistrictSelector works unchanged
    const data = cities.map((c) => ({ name: c.city, state: c.country, stations: c.stationCount, latestAQI: c.latestAQI }));
...
const getStations = async (req, res) => {
  try {
    const stations = await LocationAQI
      .find({ aqi: { $gt: 0 } })
...
module.exports = { getCurrentAQI, getForecast, getHistoricalData, getDashboard, getDistricts, getStations, triggerSync };
